...
## Coherencia Terminal-Desktop + OFM Hardening (2026-04-03)
- **Symbol format mismatch bridge→desktop**: Binance WS sends "BTCUSDT" but config uses "BTC-USD". Bridge's on_trade_hook forwarded raw Binance symbols. Desktop had to use dual lookups (`prices["BTC-USD"] || prices["BTCUSDT"]`). Fix: normalize in bridge before broadcasting (BTCUSDT→BTC-USD). Single source of truth.
- **OFM CONFIRM_TICKS=5 too slow for scalping**: 5 evals * 5s interval = 25s minimum before entry. In fast BTC moves, the opportunity is gone. Reduced to 3 (15s) — still filters noise, but responsive enough for scalping.
- **OFM no max hold time**: Positions could stay open indefinitely if score decayed slowly but never hit EXIT_SCORE_THRESHOLD. On weekends/overnight, this exposes to gap risk. Added MAX_HOLD_SEC=1800 (30 min) as safety net.
- **OFM spread-based SL vulnerable to gaps**: SL = 3x spread works during normal trading, but in low-liquidity periods spread collapses to 3 bps → SL at 9 bps, which is a noise stop. Added ATR floor: SL >= 0.3x ATR. This ensures SL respects actual volatility.
- **TP coupled to actual SL**: Previously TP = 6x spread, SL = 3x spread → always 2:1. But with ATR floor, SL might be larger. Changed TP = SL * 2.0 to maintain R:R regardless of SL source.
- **TypeScript `as any` casts hide real bugs**: `(t as any).trade_type` in PerformancePage meant the TradeData interface was incomplete. The bridge DID send trade_type (via serialize_trade), but TypeScript didn't know. Fix: add field to interface.
- **Hardcoded UI data diverges from backend**: StrategiesPage had hardcoded strategy list that didn't match config. If allocation changed in Python, desktop showed stale data. Fix: load from /api/strategies endpoint.
- **Keyboard shortcuts must use modifier keys**: Bare number keys (1-9) conflict with any text input. Using Alt+1..0 is safe — doesn't fire in inputs/textareas.
- **General pattern**: When terminal and desktop apps share a backend, ALL display data must flow from a single source (the config/API). Hardcoding values in the frontend creates drift that's invisible until someone notices the UI doesn't match reality.

## Rapid Open/Close Root Cause Analysis (2026-04-03) — CRITICAL QUANT LESSON
- **OFM cooldown bypass on SL/TP exits**: The paper simulator has TWO exit paths: (1) strategy-generated via `_check_exit()`, which updates `_last_exit_time`, and (2) SL/TP triggered via `on_price_update()`, which does NOT notify the strategy. Path 2 bypassed cooldown entirely. After SL exit, OFM saw no cooldown and immediately started building confirmation for re-entry. Fix: added `notify_external_exit()` callback pattern.
- **Microprice reversal noise exits**: Microprice is computed from CURRENT orderbook bid/ask depth, which changes ~30 times/second on Binance. A single large ask being placed shifts microprice by 5+ bps. The exit threshold was `spread_bps` (3-5 bps), meaning normal orderbook fluctuation triggered exits. Fix: added 30s minimum hold before microprice reversal exit.
- **Entry-exit asymmetry**: Entry required 3 consecutive 5s confirmations (15s), but exit required only 1 evaluation. This creates a fatal asymmetry: slow to enter, instant to exit. With microprice noise, the expected hold time was ~5-10 seconds (1-2 evals until noise triggers exit), not the intended 30-1800 seconds.
- **Multi-strategy symbol conflict**: Architecture allows each strategy its own position key (`symbol_STRATEGY`), enabling simultaneous positions on the same symbol from different strategies. While conceptually sound for hedging, with $300 capital this just doubles fees and creates contradictory signals. Fix: symbol-level position lock — only one strategy per symbol.
- **General principle**: In HFT/scalping systems, every exit path must update the cooldown. If the system has multiple ways to close a position (strategy exit, SL, TP, manual, circuit breaker), ALL paths must notify the strategy. Missing even one path creates "ghost trading" — positions opening and closing faster than the human can observe.
- **Quant rule**: Entry confirmation time must be >= exit evaluation time. If it takes 15s to confirm entry but 5s to trigger exit, the system is structurally biased toward rapid cycling and fee erosion.
- **Score invalidation exit needs hold time too**: After entry, the OBI delta EMA hasn't fully adapted to the new position context. The score can dip temporarily below EXIT_SCORE_THRESHOLD (0.15) in the first 1-2 evaluations, then recover. Without a minimum hold time on Exit 1, this creates a false exit. The same EMA lag that smooths entry signals also creates transient score dips right after entry.
- **All signal-based exits should respect MIN_HOLD**: Only SL/TP (price-based, handled by paper_sim) should fire instantly. Signal-based exits (score, counter, microprice) are NOISY and need the 30s hold buffer.

## Full System Coherence Audit #18 (2026-04-03) — LESSONS
- **WebSocket channel handlers must process ALL message types**: The global `useWebSocket` hook only handled `health` on the system channel, silently dropping `log` and `engine_error` messages. The SystemPage had its own local subscription that DID handle them, but logs were lost on page navigation. Lesson: centralize ALL message handling in the global hook, use stores for persistence.
- **Zustand fallback values hide bugs**: `equity: data.equity ?? 300` looks safe but reverts to $300 on ANY missing field (reconnect, partial message). Better: `data.equity ?? s.equity` preserves last known state.
- **EMA initialization matters for scalping**: OBI delta EMA starting at 0.0 with alpha=0.05 takes ~100s to converge. For a scalping strategy that evaluates every 5s, this means ~20 blind evaluations. Lesson: always initialize EMA to the first raw observation, not zero.
- **Dead code detection in risk systems**: Kelly Criterion was computed for every StrategyType but only OFM passed `kelly_risk_pct` to `_calc_position_size()`. MR used the fixed `risk_per_trade_pct`. The `get_kelly_risk_pct()` method existed but was orphaned from `validate_signal()`. Lesson: if a model is computed, it must flow into decisions — otherwise delete it.
- **Position sizing friction must use real notional**: `estimated_notional = capital * risk_pct * 10` is arbitrary. With $300 capital, friction is negligible. With $100k, the magic ×10 underestimates notional by 50x, making friction deduction meaningless. Lesson: compute raw size first, then calculate friction on actual notional.
- **Microprice threshold should scale with spread, not ATR**: For scalping, the relevant cost is spread (transaction friction), not ATR (volatility). ATR-based threshold becomes too easy in low vol (bad — easier to trigger, but harder to profit after fees). Spread-based scales with the actual cost of trading.
- **Alert cooldown needs atomic batch updates**: When `checkAndTrigger()` loops through rules and triggers multiple alerts, each `set()` call creates a new state snapshot but the loop continues with the old snapshot. Solution: collect all triggers, batch-update cooldowns in a single `set()`, then fire alerts.
- **Bridge message routing must be explicit**: `_broadcast_symbol_state()` drained `_pending_signals` into the `trading` channel, but some messages had `type: "log_entry"` destined for `system`. The `market_broadcast_loop` handled routing correctly, but `_broadcast_symbol_state` didn't. Lesson: every drain point must check message type.
- **Interface drift between bridge serializers and frontend types**: Bridge sends `liquidation_price`, `size_usd`, `funding_rate`, `volume_24h`, `open_interest` — all silently dropped by frontend because TypeScript interfaces didn't include them. No errors, no warnings — the data just vanishes. Lesson: generate types from serializer output, or maintain a contract test.

## Deep Audit #19 — Economics & Coherence (2026-04-03)
- **CRITICAL QUANT: OFM SL/TP was UNPROFITABLE by design**: With MIN_SPREAD_BPS=3, SL=9bps, round-trip cost=14bps. The fees EXCEED the SL distance. Net R:R after fees = 4:23 = 0.17:1. Requires 85% win rate to break even — IMPOSSIBLE to sustain. Root cause: SL calibrated on spread (microstructure) but fees calibrated on exchange cost (fixed). The two scales must be reconciled.
- **Fix: fee-based SL floor**: SL >= 2x round-trip cost = 28 bps minimum. Now net R:R = 42:42 = 1:1. Breakeven WR = 50%. Any edge above 50% generates profit. The fee floor dominates in tight-spread markets (Binance BTC), while spread/ATR floors dominate in wide-spread or volatile markets.
- **General quant principle**: ALWAYS compute net R:R AFTER all friction costs (fees + slippage + funding) BEFORE deploying any strategy. If net R:R < 0.5:1, the strategy is structurally unprofitable regardless of signal quality. This is the #1 mistake in retail algo trading.
- **Profit lock must scale with SL, not spread**: With SL=28bps and spread=2bps, profit_lock at 2x spread = 4bps locks profit too early (before even covering fees). Changed to 2x SL = 56bps, which means profit lock triggers when position has gained 2x the SL distance.
- **MAX_SL_BPS was too restrictive**: With fee floor of 28bps, the MAX_SL_BPS of 30bps left almost no room for ATR-based widening. Increased to 50bps to allow proper ATR scaling in volatile conditions.
- **SystemPage crash bug**: `setLogs([])` called a function that doesn't exist. The Clear button crashed the entire page. Fixed with `useSystemStore.setState({ logs: [] })`.
- **Candle gap cascade bug**: Bridge's candle broadcast compared each candle against the last ACCEPTED candle. After any gap > 5min, ALL subsequent candles were rejected because the gap kept growing. Chart froze permanently. Fix: compare against previous RAW timestamp, not last accepted.
...
`scripts/test_*.py` (scripts de prueba manual, no pytest) y `test_functional.py` de la raíz, fuera del alcance del
`collect_ignore` de `tests/conftest.py` (que además protege de `test_self_audit.py`, que se cuelga, y `test_p0_fixes.py`,
que falla al importar — pero solo cuando NO se nombran explícitamente). El gate del CT ejecuta `pytest tests/` (544 en 11 s)

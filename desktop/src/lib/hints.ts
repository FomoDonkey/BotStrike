...
export const HINTS = {
  ...
  liq: "Liquidation price. The trend book runs CROSS margin: liquidation is account-level, when the whole equity falls to the maintenance margin of every position, far beyond the drawdown halt — so no single price liquidates one of its positions. An isolated intraday position would liquidate at entry × (1 − 1/lev + 0.5 %) for a long, entry × (1 + 1/lev − 0.5 %) for a short; leverage 1 cannot be liquidated.",
  ...
  regime: "Intraday regime classifier on 15 m bars (ranging / trending / breakout) and how long it has held. Informational: the daily trend book decides on daily bars and does not read it.",
  ...
  drawdown: "Distance from the all-time equity peak, mark-to-market (open positions included). The peak is persisted across restarts.",
  realized: "Cash realised so far, all-time: every exit and rebalance trim net of its round-trip fee, minus the entry fees already debited on the positions still open, plus every funding settlement. Equity = initial capital + this + unrealised PnL.",

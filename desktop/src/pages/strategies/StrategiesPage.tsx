...
export function StrategiesPage() {
  ...
  const columns: Column<LeaderRow>[] = [
    ...
    { id: "wr", label: "Win rate", hint: "Round trips with a positive net PnL; n/a until the strategy closes its first full trade", sortValue: (r) => r.pf?.win_rate ?? r.edge?.win_rate ?? null, render: (r) => <span className="num">{(r.pf ? r.pf.trades > 0 : (r.edge?.n ?? 0) > 0) && typeof (r.pf?.win_rate ?? r.edge?.win_rate) === "number" ? formatPct((r.pf?.win_rate ?? r.edge?.win_rate) as number, 0) : <span className="text-text-2" title="No round trip closed by the strategy yet">n/a</span>}</span> },
    { id: "sharpe", label: "Sharpe", hint: "Annualised Sharpe of the strategy's marked daily returns; needs 30 days of history", sortValue: (r) => r.pf?.sharpe ?? null, render: (r) => <span className="num">{typeof r.pf?.sharpe === "number" ? r.pf.sharpe.toFixed(2) : <span className="text-text-2" title="Needs 30 days of marked history">n/a</span>}</span> },
    ...
    { id: "t", label: "t-stat", sortValue: (r) => r.pf?.t_stat ?? (r.edge?.verdict === "insufficient" ? null : r.edge?.t_stat) ?? null, render: (r) => { const t = r.pf?.t_stat ?? (r.edge?.verdict === "insufficient" ? null : r.edge?.t_stat); return <span className={cn("num", typeof t === "number" && t <= -2 && "text-rose", typeof t === "number" && t >= 2 && "text-mint")}>{typeof t === "number" ? t.toFixed(2) : <span className="text-text-2" title="Needs closed round trips">n/a</span>}</span>; } },
  ];

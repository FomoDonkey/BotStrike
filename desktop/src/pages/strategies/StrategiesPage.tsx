...
export function StrategiesPage() {
  ...
  const columns: Column<LeaderRow>[] = [
    ...
    { id: "wr", label: "Win rate", sortValue: (r) => r.pf?.win_rate ?? r.edge?.win_rate ?? null, render: (r) => <span className="num">{(r.pf ? r.pf.trades > 0 : (r.edge?.n ?? 0) > 0) && typeof (r.pf?.win_rate ?? r.edge?.win_rate) === "number" ? formatPct((r.pf?.win_rate ?? r.edge?.win_rate) as number, 0) : "---"}</span> },

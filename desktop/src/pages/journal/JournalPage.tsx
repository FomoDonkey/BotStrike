  ...
  return (
    <div className="flex flex-col gap-3 p-3 sm:p-4 min-w-0">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 xl:grid-cols-8 gap-2">
        ...
        <KpiCard label="Round trips" hint="Positions the strategy opened and its trailing ladder flattened. A rebalance trim realises money but is not a round trip; a manual close, a universe drop or a risk halt is a forced close and is not counted either" value={stats.closed} sub={`${stats.trims} trim${stats.trims === 1 ? "" : "s"} realised${stats.forced > 0 ? ` · ${stats.forced} forced close${stats.forced === 1 ? "" : "s"}` : ""}`} />
        {/* With 0 round trips the edge statistics cannot exist: they say so inline ("n/a · 0 round
            trips") instead of a bare "---" that read as a bug beside 31 closes (2026-09-21). */}
        <KpiCard label="Win rate" hint="Round trips with a positive net PnL" value={winRate === null ? "n/a" : formatPct(winRate, 0)} sub={winRate === null ? "0 round trips yet" : `${stats.wins} of ${stats.closed}`} />
        <KpiCard label="Profit factor" hint="Gross wins / gross losses of the closed round trips" value={pf === null ? (stats.grossWins > 0 ? "∞" : "n/a") : pf.toFixed(2)} sub={stats.closed === 0 ? "0 round trips yet" : `+${formatMoney(stats.grossWins)} / −${formatMoney(stats.grossLosses)}`} />
        <KpiCard label="Avg hold" hint="Average time from entry to the close that flattened the position — round trips and forced closes alike (a trim leaves the position open, so it has no holding time)" value={stats.avgHoldFlatSec === null ? "n/a" : formatDuration(stats.avgHoldFlatSec)} sub={stats.avgHoldFlatSec === null ? "no position closed yet" : `${stats.flattened} close${stats.flattened === 1 ? "" : "s"}${stats.forced > 0 ? ` · ${stats.closed} round trip${stats.closed === 1 ? "" : "s"} · ${stats.forced} forced` : ""}`} />
        <KpiCard label="Best / worst" hint="Best and worst closed round trips" value={stats.best ? <span className="text-mint">{formatSignedMoney(stats.best.pnl)}</span> : "n/a"} sub={stats.worst ? <span className={stats.worst.pnl < 0 ? "text-rose" : undefined}>worst {formatSignedMoney(stats.worst.pnl)}</span> : "0 round trips yet"} />

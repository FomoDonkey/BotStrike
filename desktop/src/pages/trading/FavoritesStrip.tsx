...
const Item = memo(function Item({ symbol, active, change, onSelect }: { symbol: string; active: boolean; change: number | null; onSelect: (s: string) => void }) {
...
export function FavoritesStrip({ symbol, onSelect }: { symbol: string; onSelect: (s: string) => void }) {
  ...
  return (
    <div className="flex items-center gap-1 h-10 px-1 overflow-x-auto scrollbar-none shrink-0 min-w-0">
      {FAVORITE_SYMBOLS.map((s) => <Item key={s} symbol={s} active={s === symbol} change={changes[s]} onSelect={onSelect} />)}
    </div>
  );
}
